...
topdir='/project/mojave/cesm1/LENS/lnd/month_1/'
...
for varname in varnames:
    ...
    members = [xr.open_mfdataset(i, combine='nested', concat_dim=['time'],
            coords='minimal')[[varname,'time_bounds']] for i in filelist]
    # Fix the CESM time axis
    members = [ read.fixcesmtime(members[i], timebndsvar='time_bounds').sel(time=slice("1920-01","2100-12"))
    ...
    #jja = dat.groupby('time.year').mean('time').compute()
    jja.to_netcdf(pathout+'LENS1_'+varname+'_jja.nc')
