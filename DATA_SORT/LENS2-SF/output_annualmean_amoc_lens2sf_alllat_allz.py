...
dat = xr.open_mfdataset(filelist, combine='nested', concat_dim=['M','time'], data_vars=['MOC','time_bound'])
dat = read.fixcesmtime(dat, timebndsvar='time_bound')
dat = dat.groupby('time.year').mean('time').compute()
amoc = dat.MOC.sum('moc_comp').sel(transport_reg=1) 
amoc.to_netcdf(pathout+'AAER_AMOC_alllat_allz.nc')
#----------
