...
filelist = [ sorted(glob.glob(topdir+"*.BHISTcmip6.*"+imem+"*.nc"))+
             sorted(glob.glob(topdir+"*.BSSP370cmip6.*"+imem+"*.nc"))
             for imem in memstr ] 
...
amoc.to_netcdf(pathout+'LENS2_AMOC45_am_first50members.nc')
